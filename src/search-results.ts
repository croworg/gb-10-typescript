...
export async function bookPlace(id: number, searchParams: SearchFormData): Promise<void> {
    ...
    resultsHTML += `<li class="result">
        <div class="result-container">
          ...
          <div class="result-info">
            ...
            <div class="result-info--map"><i class="map-icon"></i> ${result.remoteness
      }км от вас</div>
            <div class="result-info--descr">${result.description}</div>
            <div class="result-info--footer">
              <div>
                <button class="book-btn" data-id=${result.id
  ...
  for (let i = 0; i < bookButtons.length; i++) {
    bookButtons[i].addEventListener("click", (e) => {
      ...
      if (e.target instanceof HTMLButtonElement) {
        ...
        bookPlace(+e.target.dataset.id, getSearchParams());
      }
    });
  }
